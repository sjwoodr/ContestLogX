...
### Contest Updates
- Added the Delaware QSO Party (DEQP) contest definition (`contests/deqp.json`). First full weekend of May. Delaware stations work everyone; non-Delaware stations work Delaware stations only. Exchange is RST + 3-letter county code (NDE/KDE/SDE for New Castle/Kent/Sussex) for Delaware stations, or RST + 2-letter state/province for non-Delaware W/VE, or RST + DX otherwise. Asymmetric per-mode scoring: DE operators earn 1 point per phone QSO and 2 points per CW/Digital QSO; non-DE operators earn 10 points per phone and 20 points per CW/Digital — implemented via the engine's existing `byPrompt` per-mode points override on `stationType`. Multipliers count once per band (`multsPerBand`); non-DE operators count just the 3 Delaware counties (max 18 mults across 6 bands), while DE operators count states + provinces + DXCC entities (no DE counties — see DE-DE rule below). Power category multiplier: HP=×1, LP=×2, QRP=×3, applied to the final score via `scoring.scoreMultiplier` keyed on the `powerCategory` userPrompt. CW + SSB + non-WSJT digital modes (RTTY/PSK) only — FT8/FT4/MSK144/JT65 use a Field Day-style exchange (`1A DE`/`1A AZ`/`1A DX`) and per the contest rules must be submitted in a separate Cabrillo log generated by WSJT-X, so they are intentionally not loggable in CLX. Cabrillo CONTEST tag: `DEQP`. **DE-DE QSOs earn QSO points but do NOT credit a multiplier**, implemented via a `multAliases` rule that maps DE counties received by DE operators to a sentinel string (`__DEDESUPPRESS__`) deliberately not in `namedMults`, so the downstream membership check rejects it — points are computed from a separate path so the byPrompt-driven 1/2/2 still applies. The 50-point electronic submission bonus from the rules is awarded sponsor-side and is not added to the displayed final score (so the displayed score will be 50 short of the official sponsor-side score)
